...
$res= $db->prepare("SELECT count(*) as totalcount FROM issues WHERE team_id=:team_id and status='Open'");
...
$result = $db->prepare("SELECT * FROM issues  WHERE team_id=:team_id  and status='Open' order by id DESC limit :row1, :rowpage");
