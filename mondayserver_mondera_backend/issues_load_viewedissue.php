...
$res= $db->prepare("SELECT count(*) as totalcount FROM issues WHERE team_id=:team_id and view_time !='' order by view_time");
$res->execute(array(':team_id' =>  $teamid));
$t_row = $res->fetch();
...
$result = $db->prepare("SELECT * FROM issues  WHERE team_id=:team_id and view_time !='' order by view_time DESC limit :row1, :rowpage");
