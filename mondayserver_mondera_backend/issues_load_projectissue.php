...
$projectid = strip_tags($_POST['projectid_issue']);

$res= $db->prepare("SELECT count(*) as totalcount FROM issues WHERE team_id=:team_id and project_id=:project_id");
$res->execute(array(':team_id' =>  $teamid, ':project_id' =>$projectid));
$t_row = $res->fetch();
...
$result = $db->prepare("SELECT * FROM issues  WHERE team_id=:team_id  and project_id=:project_id order by id DESC limit :row1, :rowpage");
