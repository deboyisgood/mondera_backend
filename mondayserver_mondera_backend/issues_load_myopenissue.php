...
$teamid = strip_tags($_POST['teamid_issue']);
$userid = strip_tags($_POST['userid_issue']);

$res= $db->prepare("SELECT count(*) as totalcount FROM issues WHERE team_id=:team_id and creator_userid=:creator_userid and status='Open'");
$res->execute(array(':team_id' =>  $teamid, ':creator_userid' =>$userid));
$t_row = $res->fetch();
...
$result = $db->prepare("SELECT * FROM issues  WHERE team_id=:team_id  and creator_userid=:creator_userid and status='Open' order by id DESC limit :row1, :rowpage");
$result->bindValue(':rowpage', (int) trim($page_row_call), PDO::PARAM_INT);
$result->bindValue(':row1', (int) trim($queryid), PDO::PARAM_INT);
$result->bindValue(':team_id', trim($teamid), PDO::PARAM_STR);
$result->bindValue(':creator_userid', trim($userid), PDO::PARAM_INT);
$result->execute();
