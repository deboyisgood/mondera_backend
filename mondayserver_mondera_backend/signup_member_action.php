...
//check if team Id is found

$result = $db->prepare("SELECT * FROM users where team_identity = :team_identity and user_rank ='admin'");
...
echo  "<div id='alertdata_uploadfiles' class='well alerts alert-danger'>This Team ID Does not Exist</div>";
exit();
}
...
// check if User Email already associated with the Team Name via team Id


$result2 = $db->prepare('SELECT * FROM users where team_identity = :team_identity and email=:email');

		$result2->execute(array(
			':team_identity' => $team_id, ':email' =>$email
    ));

$nosofrows2 = $result2->rowCount();

if($nosofrows2 > 0){
echo "<div id='alertdata_uploadfiles' class='alerts alert-danger'>Your Email Address is already registered to this Team...<br></div>";
exit();
