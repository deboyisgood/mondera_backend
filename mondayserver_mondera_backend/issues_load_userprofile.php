...
$res= $db->prepare("SELECT count(*) as totalcount FROM issues WHERE team_id=:team_id and creator_userid=:creator_userid");
...
$result = $db->prepare("SELECT * FROM issues  WHERE team_id=:team_id  and creator_userid=:creator_userid order by id DESC limit :row1, :rowpage");
